...
from mythril_agent_bgm.utils.platform_utils import is_windows, is_unix, get_platform_name
...
def test_config_paths():
    ...
    if is_windows():
        ...
        expected = f"{appdata}\\mythril-agent-bgm\\"
        print(f"Windows: {expected}")
        # Verify the path is correct
        assert str(config_dir).endswith(
            "mythril-agent-bgm"
        ), f"Config dir should end with 'mythril-agent-bgm': {config_dir}"
        assert "AppData" in str(config_dir) or str(config_dir).endswith(
            "mythril-agent-bgm"
        ), f"Windows config should use AppData or fallback: {config_dir}"
    else:
        expected = "~/.config/mythril-agent-bgm/"
